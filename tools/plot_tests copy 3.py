...
def plot_test_case_real_life(plot_algorithms, normalization_algorithm, result_dir_path, plot_dir_path, nb_instances, plot_info, root_dir):
    ...
    for algorithm in algorithms:
        if algorithm == "optimal":
            continue
        sns_plot = sns.lineplot(data=results[algorithm], x="Instance", y="Normalized_cost", label=get_label(algorithm), color=get_color(algorithm), estimator=geometric_mean)
        #sns.lineplot(data=results_cmp[algorithm], x="Size", y="Normalized_cost", label=get_label(algorithm))
    if "optimal" in results:
        sns_plot = sns.lineplot(data=results['optimal'], x="Instance", y="Normalized_cost", label=get_label(algorithm), color=get_color(algorithm), linestyle="dotted", estimator=geometric_mean)
    ...
    if "xAy" in plot_dir_path:
        plt.gca().set_xticklabels(['$\psi^T \psi \quad (a)$', '$A_{\psi}^T \psi \quad (a)$', '$\psi^T \psi \quad (b)$', '$A_{\psi}^T \psi \quad (b)$', '$\psi^T \psi \quad (c)$', '$A_{\psi}^T \psi \quad (c)$'])
    else:
        plt.gca().set_xticklabels(['$\psi^T A_0 \psi \quad (a)$', '$A_{\psi}^T A_0 \psi \quad (a)$', '$\psi^T A_0 \psi \quad (b)$', '$A_{\psi}^T A_0 \psi \quad (b)$', '$\psi^T A_0 \psi_c \quad (c)$', "$A_{\psi}^T A_0 \psi \quad (c)$"])
    ...
    for algorithm in algorithms:
        if algorithm == 'naive' or algorithm == 'optimal':
            continue
        sns_plot = sns.lineplot(data=results[algorithm], x="Instance", y="Execution_time", label=get_label(algorithm, True), color=get_color(algorithm), estimator=geometric_mean)
        #plt.plot(results[algorithm].groupby('Size')['Execution_time'].mean(), line, label=get_label(algorithm), color=get_color(algorithm))
        #plt.plot(results[algorithm].groupby('Size')['Execution_time'].mean(), line, label=get_label(algorithm))
    if "optimal" in results:
        sns_plot = sns.lineplot(data=results["optimal"], x="Instance", y="Execution_time", label=get_label("optimal", True), color=get_color("optimal"), linestyle="dotted", estimator=geometric_mean)
