...
def generate_instance(file, size, instance, f_rank1, f_rank2, f_dims, rounded):
    ...
    rand1 = [f_rank1(i) for i in range(size-1)]
    rand2 = [f_rank2(i) for i in range(size-1)]
    
    rand1.sort()
    rand2.sort()

    rand1_left = rand1[::2]
    rand1_right = rand1[1::2]
    rand1_right.reverse()

    rand2_left = rand2[::2]
    rand2_right = rand2[1::2]
    rand2_right.reverse()

    rank1 = [1] + rand1_left + rand1_right + [1]
    rank2 = [1] + rand2_left + rand2_right + [1]
    # "Rounding" of TT
    if(rounded):
...
if __name__ == "__main__":
    ...
    for amount in range(50):
        for inst in range(3, 101):
            ...
            file = open("qc3/instance_{}_{}.txt".format(inst_str, amount_str), "w")
