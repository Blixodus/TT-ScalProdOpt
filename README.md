...
This repository contains reference implementations of algorithms for finding near-optimal contraction orderings in tensor-train scalar products, as presented in the paper:
...
The `src` directory contains the implementations of the ***Sweep-opt*** and $\Delta$***-opt*** algorithms, as discussed in the paper. We provide a wrapper for the [`Cotengra` library](https://github.com/jcmgray/cotengra) and include a submodule that is a fork of [`RL-TNCO`](https://github.com/NVlabs/RL-TNCO), both offering different approaches to solving the tensor network contraction ordering problem. `Cotengra` implements a state-of-the-art optimal algorithm along with multiple heuristics, while `RL-TNCO` is a reinforcement learning approach.

## Using `RL-TNCO`

This repository includes two scripts for working with `RL-TNCO`. The first one converts tensor-train network format into a pickle file, which `RL-TNCO` can then train or evaluate on. The second converts results from `RL-TNCO` evaluation into a format usable by our plotting scripts.

### Generating `RL-TNCO` training dataset

The process of generating the training dataset for `RL-TNCO` involves of two steps: first, creating the training dataset in our format, and then converting from our format into the `RL-TNCO` format, all within one pickle file. The entire training dataset can be created using the following commands:
```bash
python tools/generate_tests.py tools/rl-tnco-training.ini
# Create training file for x'y-type networks
python tools/Scripts/convert_to_rl-tnco.py datasets/train-rl-tnco/xy --file RL-TNCO/datasets/train-xy.p
# Create training file for x'Ay-type networks
python tools/Scripts/convert_to_rl-tnco.py datasets/train-rl-tnco/xAy --file RL-TNCO/datasets/train-xAy.p
```

### Converting evaluation datasets to `RL-TNCO` pickle file format

In this section, we assume that an evaluation dataset has already been created and exists in the `datasets/eval/` directory. To convert from our format of evaluation datasets to `RL-TNCO` format, use the following command:
```bash
python tools/Scripts/convert_to_rl-tnco.py datasets/eval/ --dir RL-TNCO/datasets/eval/
```
...
Before plotting or using data generated by RL-TNCO, you must convert the results back into our result format. You can use the provided conversion script for this:
```bash
python tools/Scripts/convert_to_rl-tnco.py [results_directory] [original_dataset] [output_directory]
# Example
python tools/Scripts/convert_to_rl-tnco.py RL-TNCO/results/ datasets/eval/ results-rl-tnco/
```
...
To reproduce our real-world use-case datasets from a quantum chemistry, you will need to use the [ttvibr](https://bitbucket.org/rakhuba/ttvibr/) library. Setting up this library can be somewhat challenging as it relies on an older Python version and outdated libraries. To simplify the process, we provide complete setup instructions using a [conda](https://www.anaconda.com/) environment, which is included in the `envs` directory.

Follow these steps to set up the environment and get everything working:
```bash
# Create a conda environment for ttvibr
conda env create -n ttvibr --file=envs/ttvibr.yml
conda activate ttvibr
# Install ttpy (tensor train Python package)
git clone --recurse-submodules https://github.com/oseledets/ttpy.git
cd ttpy
git checkout v1.2.0
FFLAGS="-w -std=legacy -fPIC" python setup.py install
cd ..
# Install ttvibr (the quantum chemistry library)
...
When you open the jupyter notebook, make sure to adjust the parameters to `eps = 1e-6` and `rmax = 5000` throughout the notebook. These settings will ensure that ranks are limited only by the defined precision, avoiding artificial limits. Once you’ve set the parameters, you can run all the notebook kernels. The values of `psi_new`, `A0`, and `Apsi_new` in the first iteration of the "Harmonic + Henon-Heiles correction" method were used in our paper as the real-world use-case. These values can then be converted to our tensor-train network data format using a conversion script:
```bash
